...
import {
  ...
  Globe, Calendar, ChevronRight, Star, StarOff,
  FileText, ExternalLink, MessageSquare, Clock,
...
function ClienteModal({ initial, onSave, onClose, saving }) {
  const [form, setForm] = useState({ ...EMPTY_CLIENTE, ...initial })

  const set = (k, v) => setForm(f => ({ ...f, [k]: v }))
  const toggleServicio = (s) => set('servicios',
    form.servicios.includes(s) ? form.servicios.filter(x => x !== s) : [...form.servicios, s]
  )
      ...
      <div className="bg-dark-300 border border-surface-border rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              <label className="text-gray-400 text-xs mb-1 block">Web</label>
          ...
          <div>
            <label className="text-gray-400 text-xs mb-1 block">
              <MessageSquare size={10} className="inline mr-1" />Notas
            </label>
            <textarea value={form.notas} onChange={e => set('notas', e.target.value)}
              rows={3} placeholder="Contexto, reuniones, próximos pasos..."
        ...
        <div className="px-6 py-4 border-t border-surface-border flex gap-2">
          <button onClick={onClose} disabled={saving}
            className="flex-1 px-4 py-2 rounded-lg border border-surface-border text-gray-400 hover:text-white text-sm transition-colors disabled:opacity-40">
            Cancelar
          </button>
          <button onClick={() => onSave(form)} disabled={!form.nombre.trim() || saving}
            className="flex-1 px-4 py-2 rounded-lg bg-purple-500/20 border border-purple-500/40 text-purple-300 hover:bg-purple-500/30 text-sm font-semibold transition-colors disabled:opacity-40 flex items-center justify-center gap-1.5">
...
export default function Clientes() {
  ...
  const handleSave = async (form) => {
    ...
    try {
      if (form.id) {
        const updated = await clientsApi.update(form.id, form)
        setClientes(prev => prev.map(c => c.id === form.id ? updated : c))
        if (selected?.id === form.id) setSelected(updated)
      } else {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-dark-300 border border-surface-border rounded-xl p-4">
          ...
          <p className="text-white text-2xl font-bold mt-1">{loading ? '—' : clientes.length}</p>
        ...
        <div className="bg-dark-300 border border-surface-border rounded-xl p-4">
          ...
          <p className="text-green-400 text-2xl font-bold mt-1">{loading ? '—' : activos}</p>
        ...
        <div className="bg-dark-300 border border-surface-border rounded-xl p-4">
          ...
          <p className="text-purple-400 text-2xl font-bold mt-1">
            {loading ? '—' : valorTotal > 0 ? `${valorTotal.toLocaleString('es-ES')}€` : '—'}
          </p>
        ...
        <div className="bg-dark-300 border border-surface-border rounded-xl p-4">
          ...
          <p className="text-yellow-400 text-2xl font-bold mt-1">
            {loading ? '—' : pipeline > 0 ? `${pipeline.toLocaleString('es-ES')}€` : prospectos}
          </p>
            ...
            <div className="bg-dark-300 border border-surface-border rounded-xl overflow-hidden sticky top-0">
              <div className="flex items-center justify-between px-5 py-4 border-b border-surface-border">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => deleteCliente(selected.id)}
              ...
              <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="bg-dark-400 rounded-lg p-3">
                    ...
                    <p className="text-purple-300 text-xs font-semibold">
                      {selected.valor_estimado > 0 ? `${Number(selected.valor_estimado).toLocaleString('es-ES')}€` : '—'}
                    </p>
                  </div>
                </div>
                ...
                <div className="pt-2 border-t border-surface-border">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {selected.web && (
                      <Link to="/audit" state={{ target: selected.web }}
                    ...
                    {selected.email && (
                      <Link to="/osint" state={{ target: selected.email }}
