...
4. El token de la integracion está en `backend/config.json` → sección `apis.notion` (no se publica en el repo)
...
- **API Key:** tu token de la integración (ver `backend/config.json` → `apis.notion`)
- Nombre: `Notion API`
